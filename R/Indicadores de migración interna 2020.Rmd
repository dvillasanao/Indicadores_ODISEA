...
# Se guardan los resultados
wb <- createWorkbook()
addWorksheet(wb, "Lugar de Nacimiento")
writeData(wb, 1, tabla, colNames = TRUE, rowNames = TRUE)
saveWorkbook(wb, 
             file = paste0(here::here(), "/Output/Lugar de nacimiento (Total)_Indicadores.xlsx"), 
             overwrite = TRUE)
```
          ...
          mutate(EDAD = as.numeric(.$EDAD)) %>%
           mutate(ENT_PAIS_RES_5A = case_when(.$ENT_PAIS_RES_5A %in% estados ~.$ENT_PAIS_RES_5A,
            ...
            mutate(I_Residencia = case_when(.$CVE_ENT == .$ENT_PAIS_RES_5A ~ "1",
             ...
             # Se clasifica a la población residente de la migrante
             mutate(Residentes = case_when((.$EDAD >= 5 & .$EDAD <= 130) & I_Residencia %in% "1" ~ 1,
                                           TRUE ~ 0),
             ...
             # de las tasas de emigación
              left_join(., POB_TOT %>% 
               ...
               group_by(ENT_PAIS_RES_5A) %>%
                mutate(Pob.Total.R5a = sum(M * FACTOR)) %>%
                 ungroup() %>%
