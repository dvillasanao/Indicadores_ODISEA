...
Total_Residentes_est <- svyby(~Residentes,
                               by = ~M,
                                svy_design,
                                 svytotal,
                                  vartype = c("se","cv"),
                                   drop.empty.groups = TRUE) %>%
                          cbind(confint(object = ., level = 0.90))
Total_Residentes_est %>% 
 as.data.frame() %>%
  mutate(across(where(is.numeric), ~ prettyNum(., big.mark = " ", preserve.width = "none")))

Total_Inmigrantes_est <- svyby(~Residentes,
                                by = ~M,
                                 svy_design,
                                  svytotal,
                                   vartype = c("se","cv"),
...
Total_Inmigrantes_est %>% 
 as.data.frame() %>%
  mutate(across(where(is.numeric), ~ prettyNum(., big.mark = " ", preserve.width = "none")))

## Población de 5 años y más en el estado 
est <- svyby(~I_Nacimiento,
              by = ~CVE_ENT,
               svy_design,
                svytotal,
                 vartype = c("se","cv", "ci", "ci"),
                  quantiles = 0.1, ci=TRUE,
                  drop.empty.groups = TRUE)
         cbind(confint(object = est, level = 0.90))
