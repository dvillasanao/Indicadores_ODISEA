...
tabla <- svy_design %>%
          group_by(value) %>%
           dplyr::summarise(Pob.Total = survey_total(ifelse(variable %in% "CVE_ENT", M, 0), vartype = c("se", "cv", "ci"), level = 0.90, na.rm = TRUE),
                            ...
                            TEmigracion = survey_total(ifelse(variable %in% "ENT_PAIS_NAC" & value %in% estados, numerator = Migrantes, denominator = PPob, 0), vartype = c("se", "cv", "ci"), level = 0.90, na.rm = TRUE),
