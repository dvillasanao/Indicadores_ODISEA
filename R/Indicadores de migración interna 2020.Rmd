   ...
   for(k in 1:7) {
     ...
     tabla1 <- tabla %>%
                filter(MUN_Muestra %in% Muestra[i]) %>% 
                 filter(CV_Grado %in% Grados[j]) %>% 
                  filter(TAM_MUN %in% TAM[k]) %>%
                   mutate(Diff = abs(Pob.Total_Censo - Pob.Total),
                          limite = case_when(.$Pob.Total_Censo >= (.$Pob.Total - Zcal[j] * .$Pob.Total_se) ~ 1,
                                             .$Pob.Total_Censo <= (.$Pob.Total + Zcal[j] * .$Pob.Total_se) ~ 1, 
                                             TRUE ~ NA)) %>%
                    arrange(desc(Diff))
     ...
     ylim_min_muestra = ifelse(n > 0, NA, 
                         ifelse(n == 112, -1 * min(tabla1$Pob.Total_low, tabla1$Pob.Total_Censo) - 200,
                          ifelse(n == 169, -1 * min(tabla1$Pob.Total_low, tabla1$Pob.Total_Censo) - 7100,
                                 NA)))
     
     ylim_max_muestra = ifelse(n > 0,  NA, NA)
     
     ylim_min_censo = ifelse(n > 0,  min(tabla1$Pob.Total_upp, tabla1$Pob.Total_Censo),
                             NA)
     
     ylim_max_censo = ifelse(n > 0, max(tabla1$Pob.Total_upp, tabla1$Pob.Total_Censo),
                             NA)
    
     if(n %in% c(0, 1)){
     p[[paste0(Muestra[i])]][[paste0(Grados[j])]][[paste0(TAM[k])]] <- tabla1 %>%
                                                                        ggplot(aes(x = order(factor(CVE_MUN, levels = mun)))) + 
                                                                         geom_point(aes(y = Pob.Total, 
                                                                                        color = TAM_MUN),
                                                                                     size = 2) + 
                                                                          geom_ribbon(aes(ymin = Pob.Total_low, 
                                                                                          ymax = Pob.Total_upp, 
                                                                                          color = TAM_MUN,
                                                                                          fill = TAM_MUN), 
                                                                                       alpha = 0.7) + 
                                                                           geom_point(aes(y = Pob.Total_Censo), 
                                                                                       size = 2,
                                                                                        color = "#A1A1A1") +
                                                                            geom_text_repel(aes(y = Pob.Total, 
                                                                                                label = round(Pob.Total, 1)), 
                                                                                            hjust = -1.5,
                                                                                            force = 1,
                                                                                            size = 3, 
                                                                                            angle = 45,
                                                                                            ...
                                                                                            family = "Montserrat Medium",
                                                                                            color = lineas[k]) +
                                                                             geom_text_repel(aes(y = Pob.Total_Censo, 
                                                                                                label = round(Pob.Total_Censo, 1)), 
                                                                                             hjust = 1.5,
                                                                                             force = 1,
                                                                                             size = 3, 
                                                                                             angle = 45,
    ...
    p[[paste0(Muestra[i])]][[paste0(Grados[j])]][[paste0(TAM[k])]] <- tabla1 %>%
                                                                       arrange(Pob.Total) %>%
                                                                        mutate(Etiqueta_Muestra = case_when(nrow(.) <= 30 ~ .$Pob.Total,
                                                                                                            nrow(.) > 30 &  .$limite %in% 1 & .$CVE_MUN %in% casos ~ .$Pob.Total),
                                                                               Etiqueta_Censo = case_when(nrow(.) <= 30 ~ .$Pob.Total_Censo,
                                                                                                          nrow(.) > 30 & .$limite %in% 1 & .$CVE_MUN %in% casos ~ .$Pob.Total_Censo), 
                                                                               limite_muestra = case_when(.$Pob.Total_low >= .$Pob.Total_Censo ~ .$Pob.Total_Censo - .$Pob.Total_Censo/50,
                                                                                                          .$Pob.Total_low < .$Pob.Total_Censo ~ .$Pob.Total_low - .$Pob.Total_low/50),
                                                                               limite_censo = case_when(.$Pob.Total_upp < .$Pob.Total_Censo ~ .$Pob.Total_Censo + .$Pob.Total_Censo/50,
                                                                                                        .$Pob.Total_upp >= .$Pob.Total_Censo ~ .$Pob.Total_upp + .$Pob.Total_upp/50)) %>% 
                                                                         ggplot(aes(x = order(factor(CVE_MUN, levels = mun)))) + 
                                                                          geom_line(aes(y = Pob.Total, 
                                                                                        color = TAM_MUN, 
                                                                                        size = "size1")) + 
                                                                           geom_ribbon(aes(ymin = Pob.Total_low, 
                                                                                           ymax = Pob.Total_upp, 
                                                                                           ...
                                                                                           fill = TAM_MUN), 
                                                                                        alpha = 0.2) + 
                                                                             geom_line(aes(y = Pob.Total_Censo, 
                                                                                           size = "size2"), 
                                                                                           color = "#A1A1A1", 
                                                                                       linetype = 1) + 
                                                                               ...
                                                                               theme +
                                                                                guides(size = guide_legend(override.aes = list(size = c(0.5, 0.5), color = c(lineas[k], "#A1A1A1")))) +
                                                                                 scale_size_manual(labels = c("Muestra", "Censo"), values = c(1, 0.5)) +
                                                                                  scale_color_manual(values = colores[k]) +  
                                                                                   scale_fill_manual(values = lineas[k]) +  
                                                                                    scale_y_continuous(limits = c(min(tabla1$Pob.Total_low, tabla1$Pob.Total_Censo) - x_lim, max(tabla1$Pob.Total_upp, tabla1$Pob.Total_Censo) + x_lim),
                                                                                                       labels = scales::unit_format(unit = "", scale = 1, sep = " ", big.mark = " ")) + 
                                                                                     scale_x_continuous(breaks = seq(1, nrow(tabla1), by = ceiling(nrow(tabla1)/10))) + 
                                                                                      labs(title = "Migración reciente",
                                                                                           subtitle = paste("Población Total", "\n", Muestra[i], "\n", "Precisión", Grados[j], "\n", TAM[k]),
                                                                                           y = "",
                                                                                           x = paste(n, "municipios"), 
                                                                                           size = "",
                                                                                           color = "", 
...
p
path = paste0(here::here(), "/Graficos/MR5a (Pob.Total) a nivel municipal_Tamaño de municipio.pdf") 
