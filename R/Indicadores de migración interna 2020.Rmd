...
for(i in 1:2){
  for(j in 1:3) {
    n <- tabla %>%
     ...
     x_lim <- ifelse(n < 100, 100, 
               ifelse(n >= 100 & n < 250, 500,
                ifelse(n >= 250  & n < 500, 500, 
                   ifelse(n >= 500, 20000, 
                          50 #Other cases
                          ))))
     ...
     ylim_min_muestra = ifelse(n < 30, 
                                NA,
                                 -1 * max(tabla1$Inmigrantes_low, tabla1$Inmigrantes_Censo) - 10)
     ylim_max_muestra = ifelse(n < 30, 
                                NA,
                                 -1 * min(tabla1$Inmigrantes_low, tabla1$Inmigrantes_Censo) - 10)
     
     ylim_min_censo = ifelse(n < 30, NA,
                       ifelse(n == 283,  max(tabla1$Inmigrantes_upp, tabla1$Inmigrantes_Censo) + 20,
                               min(tabla1$Inmigrantes_upp) + 40
                              ))
     ylim_max_censo = ifelse(n < 30,  NA,
                       ifelse(n == 283,  max(tabla1$Inmigrantes_upp, tabla1$Inmigrantes_Censo) + 40,
                               max(tabla1$Inmigrantes_upp) + 40))
                   ...
                   mutate(n = 1:n()) %>%
                    arrange(Inmigrantes) %>% 
                     mutate(Etiqueta_Muestra = case_when(nrow(.) <= 40 ~ .$Inmigrantes,
                                                         nrow(.) > 40 & .$limite %in% 1 & .$n < 40 ~ .$Inmigrantes),
                            Etiqueta_Censo = case_when(nrow(.) <= 40 ~ .$Inmigrantes_Censo,
                                                       nrow(.) > 40 & .$limite %in% 1 & .$n < 40 ~ .$Inmigrantes_Censo), 
  ...
  for(j in 1:3) {
   for(k in 1:7) {
    ...
    p[[paste0(Muestra[i])]][[paste0(Grados[j])]][[paste0(TAM[k])]] <- tabla1 %>%
                                                                       arrange(TEmigracion) %>%
                                                                        mutate(Etiqueta_Muestra = case_when(nrow(.) <= 40 ~ .$TEmigracion,
                                                                                                            nrow(.) > 40 &  .$limite %in% 1 & .$CVE_MUN %in% casos ~ .$TEmigracion),
                                                                               Etiqueta_Censo = case_when(nrow(.) <= 40 ~ .$TEmigracion_Censo,
                                                                                                          nrow(.) > 40 & .$limite %in% 1 & .$CVE_MUN %in% casos ~ .$TEmigracion_Censo), 
                                                                               limite_muestra = case_when(.$TEmigracion_low >= .$TEmigracion_Censo ~ .$TEmigracion_Censo - .$TEmigracion_Censo/50,
                                                                                                          .$TEmigracion_low < .$TEmigracion_Censo ~ .$TEmigracion_low - .$TEmigracion_low/50),
                                                                               limite_censo = case_when(.$TEmigracion_upp < .$TEmigracion_Censo ~ .$TEmigracion_Censo + .$TEmigracion_Censo/50,
                                                                                                        .$TEmigracion_upp >= .$TEmigracion_Censo ~ .$TEmigracion_upp + .$TEmigracion_upp/50)) %>% 
