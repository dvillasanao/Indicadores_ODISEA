    ...
    p[[paste0(Muestra[i])]][[paste0(Grados[j])]] <- tabla %>%
                                                      filter(MUN_Muestra %in% Muestra[i]) %>% 
                                                       filter(CV_Grado %in% Grados[j]) %>% 
                                                        ggplot(aes(x = order(factor(CVE_MUN, levels = ent)))) + 
                                                         geom_line(aes(y = Pob.Total, color = CV_Grado)) +
                                                         geom_ribbon(aes(ymin = Pob.Total_low, ymax = Pob.Total_upp, color = CV_Grado, fill = CV_Grado), alpha = 0.2) + 
                                                          geom_line(aes(y = Pob.Total_Censo)) + 
                                                           theme_minimal() + 
                                                            ...
                                                            scale_color_manual(values = colores[j]) +  
                                                             scale_fill_manual(values = colores[j]) +  
